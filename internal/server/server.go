...
func (s *Server) Init(adjacencyList *map[int][]int) {
	...
	for _, neighbor := range (*adjacencyList)[s.Number] {
		...
		s.ActiveNeighbors[neighbor] = s.Servers[neighbor]
	}
...
// handleMessage gère les messages reçus des autres serveurs en.
...
// handleWaveCount
func (s *Server) handleWaveCount() string {
	return ""
}

// handleProbeCount
func (s *Server) handleProbeCount() string {
	return ""
}
